...
public:
    ...
    class Producer {
    public:
        explicit Producer(MultiSourceQueue& owner) noexcept : owner_(owner), queue_(owner.allocator_), deletionQueue_(owner.allocator_) {}
    ...
    explicit MultiSourceQueue(const Allocator& allocator = Allocator()) noexcept :
        queue_(allocator), deletionQueue_(allocator), allocator_(allocator) {}
    ...
    void ApplyDeletions() noexcept {
        ...
        List<Node*> remainingDeletions(allocator_);
